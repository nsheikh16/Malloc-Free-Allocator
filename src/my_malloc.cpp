...
void find_free(size_t size, node_t **found, node_t **previous) {
  ...
  *found = NULL;
  while(current != NULL){
    if ((current->size + sizeof(node_t)) >= (size+ sizeof(header_t))){
      *found = current;
      return;
    }
    *previous = current;
...
//  The basic idea is to iterate over the free list to find a block that will fit the requested size number of bytes. It is important to
//note that you must consider the size of the block with respect to the size of the node_t struct that takes up space and
//the header_t struct that also takes up space. That is, the actual size of the block on the free list is the size field of the
//node_t struct and the node_t struct itself which must be enough to accommodate size bytes (provided as a parameter)
//plus the size of header_t .
//Once a node on the free list has been found a pointer to it must be returned using found (second parameter) and the
//previous node (if any) must be returned using previous (third parameter).
...
(*free_block)->next = t->next;
(*free_block)->size = t->size - size - sizeof(node_t);
...
void coalesce(node_t *free_block) {
  // TODO
  node_t *n = free_block;
  size_t block_size = n->size + sizeof(node_t);
if (n->next != NULL){
  node_t *nex = n->next;
  if (((char *)n) + block_size == ((char *)nex)) {
  n->size = n->size+nex->size + sizeof(node_t);
  n->next = nex->next;
   }
}
}
...
void my_free(void *allocated) {
  // TODO
  header_t *h = (header_t*)(((char *)allocated) - sizeof(header_t));
